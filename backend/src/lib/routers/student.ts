...
export function initStudentRouter(this: Ldcona): void {
  ...
  router.get("/teachers/:id", async (req: Request, res: Response) => {
    ...
    const teacher: User = await this.getFirstQueryResult(sql, req.params.id);
    if (teacher) {
  ...
  router.get(
    ...
    async (req: Request, res: Response) => {
      ...
      if (!time) return res.status(404).send({ status: "time not found" });
  ...
  router.post(
    ...
    async (req: Request, res: Response) => {
      ...
      const getTimeSql: string =
        "select id, timestamp, teacherNotes, owner from times WHERE acquired IS NULL AND owner = ? AND id = ?;";
      const time: CensoredTime = await this.getFirstQueryResult(getTimeSql, [
        req.params.id,
        req.params.timeid,
      ]);
      if (!time) return res.status(404).send({ status: "time not found" });
  ...
  router.get(
    ...
    async (req: Request, res: Response) => {
      ...
      const students: Student[] = (await this.mysqlConnection.query(sql))[0];
